...
class Logger:
    ...
    def log_scores(self, loss_names):
        ...
        loss_string = "; ".join(["%s - %.5f" % (name, value)
                                 for name, value in zip(loss_names, loss_mean)])
        loss_string = str(self.epoch).zfill(
    ...
    def visualize_rec(self, inp, out):
        ...
        imageio.imsave(os.path.join(self.visualizations_dir, "%s-rec.png" %
    ...
    def load_cpk(checkpoint_path,
                 appearance_encoder: ApearanceEncoder = None,
                 hpe_estimator: HeadPoseExpEstimator = None,
                 kp_detector: CanonicalKPDetector = None,
                 occlusion_estimator: OcclusionEstimator = None,
                 generator: OcclAwareGenerator = None,
                 discriminator: MultiScaleDiscriminator = None,
                 optimizer_appearance_encoder: Optimizer = None,
                 optimizer_hpe_estimator: Optimizer = None,
                 optimizer_kp_detector: Optimizer = None,
                 optimizer_occlusion_estimator: Optimizer = None,
                 optimizer_generator: Optimizer = None,
                 optimizer_discriminator: Optimizer = None):
        checkpoint = torch.load(checkpoint_path)

        # model settings
        if appearance_encoder is not None:
            appearance_encoder.load_state_dict(
                checkpoint['appearance_encoder'])
        if hpe_estimator is not None:
            hpe_estimator.load_state_dict(checkpoint['hpe_estimator'])
        if kp_detector is not None:
            kp_detector.load_state_dict(checkpoint['kp_detector'])
        if occlusion_estimator is not None:
            occlusion_estimator.load_state_dict(
                checkpoint['occlusion_estimator'])
        ...
        # optimizer settings
        if optimizer_appearance_encoder is not None:
            optimizer_appearance_encoder.load_state_dict(
                checkpoint['optimizer_appearance_encoder'])
        if optimizer_hpe_estimator is not None:
            optimizer_hpe_estimator.load_state_dict(
                checkpoint['optimizer_hpe_estimator'])
        if optimizer_kp_detector is not None:
            optimizer_kp_detector.load_state_dict(
                checkpoint['optimizer_kp_detector'])
        if optimizer_occlusion_estimator is not None:
            optimizer_occlusion_estimator.load_state_dict(
                checkpoint['optimizer_occlusion_estimator'])
